...
internal class PutRegisterRequestValidator : IRequestValidator<PutRegisterRequest, PutRegisterValidatedRequest>
{
    private readonly AuthorizationDbContext _authorizationDbContext;
    private readonly IClientJwkService _clientJwksService;
    private readonly ILogger<PutRegisterRequestValidator> _logger;
    private readonly IOptionsSnapshot<DiscoveryDocument> _discoveryDocumentOptions;
    private readonly ITokenRepository _tokenRepository;

    private DiscoveryDocument DiscoveryDocument => _discoveryDocumentOptions.Value;

    public PutRegisterRequestValidator(
        AuthorizationDbContext authorizationDbContext,
        IClientJwkService clientJwksService,
        ILogger<PutRegisterRequestValidator> logger,
    ...
    public async Task<ProcessResult<PutRegisterValidatedRequest, ProcessError>> Validate(PutRegisterRequest request,
        CancellationToken cancellationToken)
    {
        ...
        if (!hasEmptyIdTokenEncryptedResponseAlg || grantTypes.Contains(GrantTypeConstants.AuthorizationCode))
        {
            ...
            if (hasEmptyIdTokenEncryptedResponseAlg && !hasEmptyIdTokenEncryptedResponseEnc)
            {
                // TODO Alg must be set if Enc is set
                     ...
                     !DiscoveryDocument.IdTokenEncryptionAlgValuesSupported.Contains(request.IdTokenEncryptedResponseAlg))
            {
        ...
        if (!hasEmptyAuthorizationSignedResponseAlg || grantTypes.Contains(GrantTypeConstants.AuthorizationCode))
        {
            ...
            else if (!DiscoveryDocument.AuthorizationSigningAlgValuesSupported.Contains(request.AuthorizationSignedResponseAlg))
            {
            ...
            if (hasEmptyAuthorizationEncryptedResponseAlg && !hasEmptyAuthorizationEncryptedResponseEnc)
            {
                // TODO Alg must be set if Enc is set
                     ...
                     !DiscoveryDocument.AuthorizationEncryptionAlgValuesSupported.Contains(request.AuthorizationEncryptedResponseAlg))
            {
                // TODO invalid AuthorizationEncryptedResponseAlg
            }
            else if (!hasEmptyAuthorizationEncryptedResponseEnc &&
                     !DiscoveryDocument.AuthorizationEncryptionEncValuesSupported.Contains(request.AuthorizationEncryptedResponseEnc))
            {
        ...
        return new PutRegisterValidatedRequest
        {
            ClientId = clientId,
