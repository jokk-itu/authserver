...
{
  public static async Task<HttpResponseMessage> Login(HttpClient client, QueryString query, string username, string password, AntiForgeryToken antiForgeryToken)
  {
    var postLoginRequest = new HttpRequestMessage(HttpMethod.Post, $"connect/login{query}");
    postLoginRequest.Headers.Add("Cookie", new CookieHeaderValue(AntiForgeryConstants.AntiForgeryCookie, antiForgeryToken.Cookie).ToString());
